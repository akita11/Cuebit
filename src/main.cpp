...
{
	if (f == 0) digitalWrite(MOTOR_nSLEEP, HIGH);
	else digitalWrite(MOTOR_nSLEEP, LOW);
}
void enableLED(uint8_t f)
{
...
uint8_t changeMotorPattern(uint8_t m, uint8_t dir)
{
	...
	else if (dir == MOTOR_DIR_CCW) m = (m - 1) % MOTOR_STEP;
	return(m);
}
